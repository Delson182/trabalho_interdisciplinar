...
public class ServidorSetorDAO {
	...
	public ServidorSetor recuperaPorIdServidor(Integer servidor_id) {
		ServidorSetor servidorSetor = new ServidorSetor();
		try {
			EntityManager em = JPAUtil.getEntityManager();
			Query query = em.createQuery("FROM servidor_setor where servidor_fk=:id");
			query.setParameter("id", servidor_id);
			servidorSetor = (ServidorSetor) query.getSingleResult();
			em.close();
		} catch (Exception e) {
			e.printStackTrace();
		}
